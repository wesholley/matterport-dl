...
- **Stuck on "LOADING" with no console error, but the Network tab shows a `graph` request that never completes (or fails as "empty response")**: this means Matterport's live app is asking for a GraphQL query this tool doesn't yet know about. Matterport adds these occasionally, so this can recur years apart. The fix has two parts, both already applied in this copy, but worth knowing if it happens again with a *different* unrecognized query:
  1. `do_GraphRequest()` (in `matterport-dl.py`) now always sends *some* response instead of silently hanging on an unrecognized query — so worst case this now degrades to a console error instead of an infinite hang.
  2. To fix it properly rather than just degrade gracefully: open the browser's Network tab, find the failing/hanging `api/mp/models/graph?operationName=...` request, copy its query-string, and add it as a new entry to the `GRAPH_DATA_REQ` dict near the top of `matterport-dl.py` (replace the `modelId` value with `[MATTERPORT_MODEL_ID]`, matching the existing entries). Then run a refresh (see above) to actually fetch and cache real data for it, and restart the server.
- **A file 404s in `server.log`**: not necessarily a problem. A few optional feature chunks (VR/plugin-related, not core viewing) are known to be unavailable even directly from Matterport's own servers and don't affect the tour itself.
...
3. A fix for `do_GraphRequest()` silently hanging (no HTTP response at all) on a GraphQL query it didn't recognize, plus added recognition of `GetModelAssets` specifically (a query Matterport's live app now makes on load that this tool didn't know about) — this was the cause of a second, later blank-screen issue after the first fix. See the troubleshooting entry above for what to do if a *different* unrecognized query shows up in the future.
4. The `curl_cffi`-independent viewing behavior and the pinned `requirements-lock.txt` described above.

These are preserved on **https://github.com/wesholley/matterport-dl** (`main` branch has everything). The bug fixes (not the archival-specific changes) have also been submitted upstream as [rebane2001/matterport-dl#206](https://github.com/rebane2001/matterport-dl/pull/206) — fix #3 above still needs to be added there.
